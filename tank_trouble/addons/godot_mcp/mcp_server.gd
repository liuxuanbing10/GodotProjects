...
# Editor interface injected from the EditorPlugin
var _editor_interface: EditorInterface = null


func set_editor_interface(ei: EditorInterface) -> void:
	_editor_interface = ei
	# Inject into existing tool instances (if _register_tools already ran)
	for category in _tools:
		var executor = _tools[category]
		if executor.has_method("set_editor_interface"):
...
func _register_tools() -> void:
	...
	# Inject editor interface into all tools (in case set_editor_interface was called before _ready)
	if _editor_interface:
		for category in _tools:
			var executor = _tools[category]
			if executor.has_method("set_editor_interface"):
				executor.set_editor_interface(_editor_interface)
